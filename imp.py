...
# ─── 1. 建立 FastAPI 網頁伺服器(用於 Render 的健康檢查與防休眠) ───
app = FastAPI()
...
@app.head("/")
async def home_head():
    return None  # 完美支援 UptimeRobot 的 HEAD 盲戳檢查,防 502 錯誤

# ─── 2. Discord 機器人核心與多伺服器狀態機 ───
...
@bot.event
async def on_ready():
    print(f'🤖 誰是臥底核心狀態機已連線:{bot.user.name}')
...
async def vote_player(ctx, member: discord.Member = None):
    game = get_game_state(ctx.guild.id)
    if not game["is_active"] or ctx.author not in game["players"]: 
        return
...
async def main():
    ...
    # 網頁監聽與機器人同時運作,相容 Render 平台檢測,確保 24H 不休眠斷線
